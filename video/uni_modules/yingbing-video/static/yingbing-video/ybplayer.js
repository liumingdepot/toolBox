class YBPlayer {
	...
	_setControlsView (value) {
		if ( value == 'play' && this._querySelector('ybplayer-icon-play') ) {
			...
			if ( !this._isDrag ) {
				if ( this.timeShow ) {
					let currentTime = this.video ? this.video.currentTime && this.video.currentTime != 'Infinity' ? this.video.currentTime : 0 : 0;
					let duration = this.video ? (this.duration || (this.video.duration && this.video.duration != 'Infinity' ? this.video.duration : 0) ) : 0;
