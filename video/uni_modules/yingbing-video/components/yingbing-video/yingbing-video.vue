...
<script lang="renderjs" type="module" module="ybVideo">
	...
	export default {
		...
		methods: {
			...
			_initScript (dataId) {
				if ( dataId == 'hls' ) {
					if ( !this._getScriptDomByDataId(dataId) ) {
						this._initHLS().then(() => {
							this._initScript('flv')
						})
					}
				}
				if ( dataId == 'flv' ) {
			...
			_initHLS () {
				return new Promise(resolve => {
					...
					script.src = './uni_modules/yingbing-video/static/yingbing-video/hls.min.js'
					script.onload = () => {
						resolve()
					}
			...
			_initFLV () {
				return new Promise(resolve => {
					...
					script.src = './uni_modules/yingbing-video/static/yingbing-video/flv.min.js'
					script.onload = () => {
						resolve()
					}
					document.body.appendChild(script)
			...
			_initYBBarrage () {
				return new Promise(resolve => {
					...
					script.src = './uni_modules/yingbing-video/static/yingbing-video/ybbarrage.js'
					script.onload = () => {
						resolve()
					}
					document.body.appendChild(script)
			...
			_initYBPlayer () {
				return new Promise(resolve => {
					...
					script.src = './uni_modules/yingbing-video/static/yingbing-video/ybplayer.js'
					script.onload = () => {
						resolve()
					}
					document.body.appendChild(script)
