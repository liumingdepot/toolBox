...
* 3、支持弹幕功能
* 4、该组件需要给定高度
* 5、该插件采用script引入js的方式，但uni_modules中添加static文件夹需要高版本的插hbuilderX才能生效，我是用的hbuilderX版本是3.5.3，仅供大家参考，如果是低版本的hbuilderX，并且不能升级版本的话，只有将插件内的static文件夹中的文件复制到项目根目录的static文件夹中，然后在yingbing-video.vue文件中修改script引入的路径

```javascript
	//css文件引入路径
    link.href = './uni_modules/yingbing-video/static/ybplayer.css'
    //弹幕js文件引入路径
    script2.src = './uni_modules/yingbing-video/static/ybbarrage.js'
    //hls.min.js文件引入路径
    script3.src = './uni_modules/yingbing-video/static/hls.min.js'
    //flv.min.js文件引入路径
    script4.src = './uni_modules/yingbing-video/static/flv.min.js'
    //播放器js文件引入路径
    script1.src = './uni_modules/yingbing-video/static/ybplayer.js'
```

* 5、如果使用的是高版本的hbuilderX，但还是出现YBPlayer报错的问题，请确认安装插件后有没有再次编译过项目，如果再次编译过项目还是不行，请先确定项目里有没有unpackage/dist/dev/app-plus/uni_modules/yingbing-video/staitc/ybplayer.js文件，再来提问
* 6、有什么不懂可以加群 1087735942
