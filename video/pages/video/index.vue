<template>
	<view class="content">
		<u-navbar :title="videoItem.vod_name" ref="navBar" bgColor="#d8f2d0" @leftClick="leftClick" placeholder>
		</u-navbar>
		...
		<view class="main">
			<view class="top">
				...
				<view class="right">
					<view class="title">{{videoItem.vod_name}}</view>
					<view class="ziyuan">
						<view>{{videoItem.typeName}}资源</view>
						<view class="fg">|</view>
						<view>{{videoItem.type_name}}</view>
						<view class="fg">|</view>
						<view>{{videoItem.vod_area}}</view>
					</view>
					<view class="ziyuan">{{videoItem.vod_pubdate}}</view>
					<view class="grid">
						<view class="text">导演:</view>
						<view class="con">{{videoItem.vod_director}}</view>
					</view>
					<view class="grid">
						<view class="text">主演:</view>
						<view class="con">{{videoItem.vod_actor}}</view>
					</view>
				</view>
			</view>
			<view class="read-more" v-if="videoItem.vod_blurb">
				<u-read-more color="#24C880" showHeight="50" textIndent='0' :shadowStyle="shadowStyle">
					<view>{{videoItem.vod_blurb.replace(/\ +/g, "")}}</view>
				</u-read-more>
			</view>


			<view class="titleBox">
				<view class="textZ">源选择</view>
				<view class="text">播放失败请切换其他源</view>
			</view>
		...
		<!-- 选集 -->
		<u-popup :show="show2" mode="bottom" :round="10" closeable @close="show2 = false">
			<view style="padding: 24rpx;line-height: 1.5;">选集</view>
			<scroll-view class="scrollY" scroll-y="true" v-if="videoItem.vod_name">
				<!-- 电视剧 -->
				<view class="actives actives2" v-if="videoType == 1">
					<view class="item" :class="jiIndex == index ?'active':''"
						v-for="(item,index) in videoList[current].videoList" :key="index"
	...
	export default {
		...
		methods: {
			...
			getVideoInit() {
				this.tuijianList = []
				// 获取缓存
				const videoItem = uni.getStorageSync('videoItem');
				// 设置标题
			...
			async cnxh(videoItem) {
				console.log(videoItem.typeUrl);
				for (let i = 0; i < videoItem.typeUrl.length; i++) {
					const itemUrl = videoItem.typeUrl[i]
					const src = itemUrl.url + `${itemUrl.url}?&ac=videolist&t=${itemUrl.type_id}`
					try {
						const res = await cnxhVideo(src)
						for (let item of res.list) {
							const temp = this.tuijianList.find(e => e.vod_en == item.vod_en && e.vod_name == item
								.vod_name)
							if (!temp) {
								this.tuijianList.push(item)
							}
							this.$forceUpdate()
						}
					} catch (err) {
